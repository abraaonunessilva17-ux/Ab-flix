...
import {buscarPorFiccaoCientifica} from '../../../../controllers/filmControllers'; //Importamos a função 'buscarPorFiccaoCientifica' que foi criada no arquivo que concentra todas as funções de filtro do projeto Filmes, a 'controllers.ts'.

export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    await connectMongo(); //Conecta ao banco de dados (crucial em cada rota da API do Next.js).

    if(req.method === 'GET') { //Se for um 'get'. Usamos esse método pois mesmo sendo uma busca, mas é uma 'leitura'.
       return buscarPorFiccaoCientifica(req, res);
    }
